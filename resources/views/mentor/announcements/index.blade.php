...
<div class="min-h-screen bg-slate-50 py-10">
    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 space-y-8">
        <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <a href="{{ route('mentor.dashboard') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-100">Kembali ke Dashboard</a>
        </div>
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="mb-6 flex items-center justify-between">
                ...
                @if ($editingAnnouncement)
                    <a href="{{ route('mentor.announcements.index') }}" class="text-sm font-semibold text-blue-600 hover:text-blue-700">Batal edit</a>
                @endif
            </div>

            <form method="POST" action="{{ $editingAnnouncement ? route('mentor.announcements.update', $editingAnnouncement) : route('mentor.announcements.store') }}" class="grid gap-4 md:grid-cols-2">
                @csrf
                @if ($editingAnnouncement)
                    @method('PUT')
                @endif
                    ...
                    <div class="px-6 py-5">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('mentor.announcements.index', ['edit' => $announcement->id]) }}" class="rounded-lg border border-blue-200 px-3 py-1.5 text-sm font-semibold text-blue-700 hover:bg-blue-50">Edit</a>
